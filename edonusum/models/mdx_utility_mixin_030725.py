...
class MdxUtilityMixin(models.AbstractModel):
    ...
    def generate_note_for_invoice(self, invoice_record):
        ...
        if currency_code != "TRY":
            ...
            final_note += f" (1 {currency_code} = {exchange_rate:.4f} {home_currency_unit}/{home_currency_subunit})"
    ...
    def generate_invoice_xml(self, invoice_record):
        ...
        for invoice_record in invoice_record.with_context(lang="tr_TR"):
            ...
            if efatura_turu != "EARSIV":
                ...
                xml_string += '<cbc:ID schemeID="VKN_TCKN">0012120959</cbc:ID>'
                xml_string += "<cac:SignatoryParty>"
                xml_string += "<cac:PartyIdentification>"
                xml_string += '<cbc:ID schemeID="VKN">0012120959</cbc:ID>'
                xml_string += "</cac:PartyIdentification>"
            ...
            else:  # TODO : remove this else block after testing
                xml_string += '<cbc:ID schemeID="VKN">0012120959</cbc:ID>'
            xml_string += "</cac:PartyIdentification>"
            xml_string += "<cac:PartyIdentification>"
            xml_string += (
        ...
        post_string = f"""
            ...
            <soapenv:Body>
                <ser:gidenBelgeDurumSorgulaExt>
                    <vergiTcKimlikNo>3680272293</vergiTcKimlikNo>
                    <parametreler>
                        <belgeNo>{oid}</belgeNo>
    ...
    def generate_waybill_xml(self, waybill_record):

        # MdxUtilityMixin.check_license(self)

        partner_id = waybill_record.partner_id
        ...
        if partner_id.type != "delivery":
            ...
            partner_delivery_address_id = partner_id
        ...
        if eski_irsaliye_no and eski_irsaliye_no.startswith(irsaliye_seri_code):
            ...
            irsaliye_no = f"{irsaliye_seri_code}{year}{str(irsaliye_seri_index).zfill(9)}"  # Index'e 12 basamağa kadar sıfır ekliyoruz
        ...
        uuid = waybill_record.uuid or self.generate_uuid()
        if not waybill_record.uuid:
        ...
        xml_string += "<cac:Party>"
        xml_string += "<cbc:WebsiteURI>" + company_website + "</cbc:WebsiteURI>"
        xml_string += "<cac:PartyIdentification>"
        xml_string += '<cbc:ID schemeID="VKN">' + company_vkn + "</cbc:ID>"
        xml_string += "</cac:PartyIdentification>"
        xml_string += "<cac:PartyName>"
        ...
        for line_index, line in enumerate(waybill_record.move_ids, start=1):
            ...
            xml_string += "<cbc:Name>" + line.product_id.name + "</cbc:Name>"
            xml_string += "<cac:BuyersItemIdentification>"
            xml_string += "<cbc:ID></cbc:ID>"
            xml_string += "</cac:BuyersItemIdentification>"
            xml_string += "<cac:SellersItemIdentification>"
            xml_string += "<cbc:ID>" + line.product_id.name + "</cbc:ID>"
            xml_string += "</cac:SellersItemIdentification>"
                ...
                <soapenv:Body>
                    <ser:gelenBelgeleriListeleExt>
                        <parametreler>
                            ...
                            <vergiTcKimlikNo>{vkn}</vergiTcKimlikNo>
                            <gelisTarihiBaslangic>{year}0101000000000</gelisTarihiBaslangic>
