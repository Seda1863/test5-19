...
class PaymentProvider(models.Model):
    ...
    def _send_api_request(
        ...
        """Send an HTTP request to the Iyzico API (sandbox or production)."""
        import requests
        self.ensure_one()

        # 1️⃣ Build URL ve header
        url = self._build_request_url(endpoint)
        headers = self._build_request_headers(method, endpoint, json or data or {})

        _logger.info("[Iyzico] Sending API request: %s %s", method, url)
        _logger.debug("[Iyzico] Headers: %s", headers)
        _logger.debug("[Iyzico] Payload: %s", json or data)

        # 2️⃣ HTTP isteği gönder
        try:
            ...
            _logger.error("[Iyzico] Connection error: %s", e)
            raise ValidationError(_("Could not reach the Iyzico API."))

        # 3️⃣ HTTP yanıt kodunu kontrol et
        if response.status_code != 200:
            error_msg = f"Iyzico API HTTP {response.status_code}: {response.text}"
            _logger.error(error_msg)
            raise ValidationError(error_msg)

        # 4️⃣ Yanıtı ayrıştır
        try:
            result = self._parse_response_content(response)
        except Exception as e:
            _logger.error("[Iyzico] Invalid response: %s", e)
            raise ValidationError(_("Invalid response from Iyzico API."))

        # 5️⃣ Log ve dönüş
        _logger.info("[Iyzico] API Response: %s", result)
        return result
